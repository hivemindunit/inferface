  ...
  return (
    <main className="min-h-screen bg-zinc-950 text-zinc-100">
      ...
      <section className="flex flex-col items-center justify-center px-6 pt-32 pb-24 text-center">
        <h1 className="text-6xl sm:text-7xl font-bold tracking-tighter bg-gradient-to-b from-white to-zinc-400 bg-clip-text text-transparent">
          inferface
        </h1>
        <p className="mt-4 text-lg sm:text-xl text-zinc-400 max-w-xl">
        ...
        <div className="mt-8 flex gap-4">
          ...
          <a
            ...
            className="flex items-center gap-2 rounded-xl border border-zinc-700 bg-zinc-900 px-6 py-3 text-sm font-medium text-zinc-300 hover:border-zinc-500 hover:text-white transition-colors"
          >
      ...
      <section className="px-6 pb-20">
        <div className="mx-auto max-w-3xl text-center">
          <div className="inline-flex items-center gap-3 rounded-2xl border border-emerald-800/50 bg-emerald-950/30 px-8 py-5">
            <div className="flex items-center justify-center h-10 w-10 rounded-xl bg-emerald-600/20 text-emerald-400">
              <svg className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z" clipRule="evenodd" />
              </svg>
            </div>
            <div className="text-left">
              <div className="text-sm font-semibold text-emerald-300">Zero runtime dependencies</div>
              <div className="text-xs text-zinc-500">
                <code className="text-emerald-400/70">@inferface/hooks</code> — &lt;10KB gzipped. No transitive deps. Just React.
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {DEMO_CARDS.map((card) => (
              <Link
                key={card.href}
                href={card.href}
                className="group flex flex-col rounded-2xl border border-zinc-800 bg-zinc-900/50 p-6 hover:border-zinc-600 hover:bg-zinc-900 transition-all"
              >
                <div className="text-xs font-mono text-emerald-400 mb-2">{card.hook}</div>
                <h3 className="text-lg font-semibold text-zinc-100 group-hover:text-white transition-colors">
                  {card.title}
                  <span className="ml-1 opacity-0 group-hover:opacity-100 transition-opacity" aria-hidden="true">&rarr;</span>
                </h3>
                <p className="mt-2 text-sm text-zinc-500 flex-1">{card.description}</p>
                <pre className="mt-4 rounded-lg bg-zinc-950 border border-zinc-800 p-3 text-[11px] leading-relaxed overflow-hidden">
                  <code className="text-emerald-300/80">{card.code}</code>
                </pre>
              </Link>
            ))}
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-zinc-800 bg-zinc-900/50">
                  <th className="px-6 py-3 text-left font-medium text-zinc-400">Hook</th>
                  <th className="px-6 py-3 text-left font-medium text-zinc-400">Description</th>
                  <th className="px-6 py-3 text-left font-medium text-zinc-400">Key Returns</th>
                </tr>
              </thead>
              <tbody>
                {HOOKS_TABLE.map((hook) => (
                  <tr key={hook.name} className="border-b border-zinc-800/50 last:border-0">
                    <td className="px-6 py-3">
                      <code className="text-emerald-400 text-xs font-mono">{hook.name}</code>
                    </td>
                    <td className="px-6 py-3 text-zinc-400">{hook.description}</td>
                    <td className="px-6 py-3">
                      <code className="text-zinc-500 text-xs font-mono">{hook.returns}</code>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <footer className="border-t border-zinc-800 px-6 py-8">
        <div className="mx-auto max-w-5xl flex flex-col sm:flex-row items-center justify-between gap-4 text-sm text-zinc-600">
          <div className="flex items-center gap-3">
            <span className="font-semibold text-zinc-500">inferface</span>
            <span className="rounded-full border border-zinc-800 px-2 py-0.5 text-xs text-zinc-600">0.0.1-alpha</span>
          </div>
          <div className="text-zinc-700">built with inferface</div>
          <a
            href="https://github.com/hivemindunit/inferface"
            target="_blank"
            rel="noopener noreferrer"
